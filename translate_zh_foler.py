...
TRANSLATE_SYSTEM_PROMPT = """你是一名专业的翻译官。你的任务是将用户输入的视频里面每句话都翻译成[中文]。如果遇到专业术语，请保持原样不变。确保翻译考虑到字幕的上下文，以准确传达原意。不要说对不起或者其他话语，也不要说抱歉和多余的其他话。可以结合下面的视频概要理解所要翻译的具体内容:
{}
"""
...
def translate_srt(input_file, output_file, summary_text):
    ...
    for sub in subs:
        translated_text = client.generate(model=MODEL_NAME, prompt=sub.text, system=TRANSLATE_SYSTEM_PROMPT.format(
            summary_text), stream=False, options=Options(temperature=0.1, num_gpu=-1, num_thread=16, num_ctx=16384))['response']
        print(
